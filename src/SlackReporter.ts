...
class SlackReporter implements Reporter {
  ...
  async onEnd(): Promise<void> {
    ...
    const slackClient = new SlackClient(
      new WebClient(this.slackOAuthToken || process.env.SLACK_WEBHOOK_URL, {
  ...
  preChecks(): { okToProceed: boolean; message?: string } {
    ...
    if (!this.slackOAuthToken && !process.env.SLACK_WEBHOOK_URL) {
      return {
        okToProceed: false,
        message: '❌ Neither slackOAuthToken nor process.env.SLACK_WEBHOOK_URL were found',
      };
    }
